...
/*
        ...
        Various command can invoke the same backend handler but the command 
        helps to diiferentiate between various codes that get executed 

         > extract the command codes 
         > and use it later to implement the various clis
...
isis_config_handler(param_t *param, 
    ...
    TLV_LOOP_BEGIN(tlv_buf, tlv){
        // comparing the leaf ids i:e strings 
...
// conf node <node-name> proto isis interface all --> to enable the proto on all the interfaces of the nodes 
// conf node <node-name> proto isis interface <interface0-name> --> enables the protocol on the selected interface 
...
isis_intf_config_handler(param_t *param, 
    ...
    switch(cmdcode) {
        case CMDCODE_CONF_NODE_ISIS_PROTO_INTF_ENABLE:
            // getting the interface name by the node name
...
isis_show_handler(param_t *param, 
    ...
    /* 
    The TLV buffer shares the data that is passed by the user which is used to 
    obtain the command codes based on it*/
    cmdcode = EXTRACT_CMD_CODE(tlv_buf);
    ...
    /* getting the value of the node by the node name and the associated topology
    , the topo is a global variable representing the topology and the node name
    is the name of the node */
    node = node_get_node_by_name(topo, node_name);
    

    /* 
        positive and the negative command codes 
    */
    switch(cmdcode) {
...
isis_config_cli_tree(param_t *param) {

    {
        ...
        */
        ...
        // CLI integration hiearchy
        static param_t isis_proto; 
	    init_param(
                &isis_proto, // address of the current param 
                CMD,  // CMD for command param and LEAF for leaf poram 
                "isis", // name of the param that is getting displayed in the command line
                isis_config_handler, // callback function i:e pointers to the application function
                0, // application specific function which is used for validating the data from the user 
                INVALID, // data type for the CMD params will be always NULL
                0, // name of the leaf param that is used for parsing the application code to find the value passed by the user 
                "isis protocol"); // helper string

        // register the parameter  
        // Here the isis_proto becomes the leaf of the parent param 
        // The param indicates the command protocol
	    libcli_register_param(param, &isis_proto);

        // setting the command code
        // The command code is used to sent to the command code to the callback function 
...

        {
            ...
            libcli_register_param(&isis_proto, &interface);
            {
                ...
                */
                ...
                // The if-name is used in the callback handler to extract the info about the specific interfaces 
                init_param(&if_name, LEAF, 0, isis_intf_config_handler, 0, STRING, "if-name", // if-name is the interface name 
                        ("Interface Name"));
