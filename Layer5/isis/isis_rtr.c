...
isis_init(node_t *node ) {
    ...
    /* Registering the interested packets in the protocol stack based on the packet classification rule  */
    tcp_stack_register_l2_pkt_trap_rule(
			node, isis_lsp_pkt_trap_rule, isis_pkt_recieve);

    /* allocate the memory for the current requested nodes */
...
*/
    nfc_intf_register_for_events(isis_interface_updates); // all the interface changes get registered by this API through the 
							  // invoking of the API isis_interface_updates and feed all the 
							  // relevant changes as the args to this function
