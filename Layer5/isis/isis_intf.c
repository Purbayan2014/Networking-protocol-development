...
/*
   initialize the isis_interface object that is pointed by the interface_t
*/    
...
/*
   Following this architecture principles 
      > only enable at the node level 
      > else through an exception
      > if the protocol is enabled then do nothing
      > finally enabling the protocol on the interface 
*/

    // getting the pointer to the attached node of the interface 
    // then fecthing the information
